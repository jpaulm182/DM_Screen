...
class SpellReferencePanel(BasePanel):
    ...
    def __init__(self, app_state, panel_id=None):
        ...
        # Create UI directly in __init__ rather than in _setup_ui
        # Main layout
        ...
        # Load spells after UI is fully set up
        self._load_spells()
    ...
    def _setup_reference_tab(self):
        ...
        try:
            ...
            print(f"Error setting up reference tab: {e}")
            # Create a minimal layout as fallback
            fallback_layout = QVBoxLayout(self.reference_tab)
            fallback_layout.addWidget(QLabel("Error loading spell reference. Please restart the application."))
    ...
    def _setup_tracker_tab(self):
        ...
        try:
            ...
            print(f"Error setting up tracker tab: {e}")
            # Create a minimal layout as fallback
            fallback_layout = QVBoxLayout(self.tracker_tab)
            fallback_layout.addWidget(QLabel("Error loading spell tracker. Please restart the application."))
    ...
    def _update_spell_table(self):
        ...
        try:
            if not hasattr(self, 'spell_table') or not self.spell_table:
                return
            ...
            for idx, spell in enumerate(self.filtered_spells):
                ...
                level = spell['level']
                level_text = "Cantrip" if level == 0 else str(level)
            ...
            print(f"Error updating spell table: {e}")
    ...
    def _filter_spells(self):
        ...
        try:
            ...
            print(f"Error filtering spells: {e}")
    
    def _show_spell_details(self, row, column):
        """Show details for the selected spell"""
        try:
            if not hasattr(self, 'spell_name') or not self.spell_name:
                return
            ...
            print(f"Error showing spell details: {e}")
    ...
    def _use_spell_slot(self, level):
        ...
        try:
            ...
            print(f"Error using spell slot: {e}")
    ...
    def _restore_spell_slot(self, level):
        ...
        try:
            ...
            print(f"Error restoring spell slot: {e}")
    ...
    def save_state(self):
        ...
        # We'll just save the current tab for now
        if hasattr(self, 'tabs'):
            return {"current_tab": self.tabs.currentIndex()}
        return {}
    
    def restore_state(self, state):
        """Restore the panel state"""
        if not state or not hasattr(self, 'tabs'):
            return

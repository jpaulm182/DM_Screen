...
class CombatTrackerPanel(BasePanel):
    ...
    def _handle_resolution_result(self, result, error):
        """Handle the final result from combat resolution"""
        # Re-enable button
        self.fast_resolve_button.setEnabled(True)
        self.fast_resolve_button.setText("Fast Resolve")
             
        if error:
            QMessageBox.critical(self, "Combat Resolution Error", str(error))
            return
             
        if not result:
            QMessageBox.warning(self, "Combat Resolution", "Combat resolution completed but returned no results.")
            return
             
        # Log the summary to combat log
# app/ui/panels/combat_tracker_panel.py - Combat tracker panel
"""
Combat tracker panel for managing initiative and combat encounters
...
class CombatTrackerPanel(BasePanel):
    ...
    def _fast_resolve_combat(self):
        ...
        # Call the resolver with proper error handling
        try:

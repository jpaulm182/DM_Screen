...
Run this before starting the main application to enable detailed monitoring.
"""
...
def main():
    """Main function to apply all monitoring patches"""
    logger.info("Starting LLM call monitoring setup")
    
    success = patch_llm_service()
    if success:
        logger.info("Successfully patched LLM service")
    else:
        logger.error("Failed to patch LLM service")
    
    success = patch_combat_resolver()
    if success:
        logger.info("Successfully patched CombatResolver")
    else:
        logger.error("Failed to patch CombatResolver")
    
    success = monitor_ui_updates()
    if success:
        logger.info("Successfully patched UI update methods")
    else:
        logger.error("Failed to patch UI update methods")
    
    logger.info("LLM monitoring setup complete. All calls will be logged to the output directory.")
    logger.info(f"Run the application normally with 'python3 main.py' and check logs in {output_dir}")
